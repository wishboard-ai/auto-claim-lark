...
机器人（本服务）
      ...
      ├─ 2.5 二维码交叉校正（增值税发票：解析票面二维码，校正发票号码/代码/开票日期并校验金额）
...
增值税发票上阿拉伯数字有三个：「金额（不含税）」「税额」「价税合计（含税）」，模型容易取错列；而**大写金额只对应价税合计**，且大写字形（壹/贰/叁…）不像 3/8、0/6 那样易混。因此识别时会额外抽取 `amountInWords`（票面大写原文），用 `src/invoice/chineseAmount.ts` 解析成数字后与小写勾稽：

- 大写 ≈ 小写价税合计 → 相互印证，不改；
- 大写 ≈ 不含税 + 税额 → 大写即价税合计，用它校正小写（典型场景：模型把「金额」列当成了合计）；
...
增值税发票（含全电发票）左上角的二维码是**明文机器编码**，包含发票代码、发票号码、不含税金额、开票日期等。这些字段（尤其号码/代码/日期）比 OCR 读票面文字更可靠。

识别增值税发票后，会额外解析票面二维码（`src/invoice/qrcode.ts`，用 `@napi-rs/canvas` 解码像素 + `jsQR` 识别），并交叉校正：

- **发票号码 / 发票代码 / 开票日期**：OCR 缺失或与二维码不一致时，采用二维码值（更可信）。
- **金额校验**：二维码第 5 段金额（实测全电发票为**含税价税合计**）。不预设含税/不含税，而是与 OCR 的「价税合计 / 不含税金额 / 税额」逐一勾稽：若二维码金额与 OCR 价税合计一致则相互印证；若等于「不含税 + 税额」则据此校正/补全价税合计；OCR 完全无金额时用二维码金额兜底；对不上时不据此改金额，避免算错。

该步骤**只增强不破坏**：无二维码、解析失败、或非增值税发票时，静默跳过并沿用 OCR 结果，绝不阻断识别。图片型/文本型 PDF 都会尝试（文本型 PDF 会额外栅格化首页仅用于取二维码）；火车票、出租车票不适用此格式，自动跳过。校正动作会记录到日志（`二维码交叉校正：…`）。
