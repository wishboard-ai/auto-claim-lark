...
1. 用户先回复「借款核销」，上传实际消费发票并填写核销事由；机器人按申请人查询 `LOAN_APPROVAL_CODE` 下近 `LOAN_LOOKBACK_DAYS` 天已通过且仍有余额的付款申请。超过飞书单次查询上限的回看范围会自动拆成不超过 30 天的窗口，并自动处理分页。
2. 只有一笔可核销借款时自动关联；有多笔时展示审批编号、借款金额、剩余金额、事由和实际借款日期，由用户回复序号选择。
